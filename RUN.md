...
> **汇总口径**：JORC 资源表把同一份资源量按 In-situ / Stockpiles / 全矿三种口径各列
> 一遍，逐行相加会把同一份资源量算两遍。解析器会剔除 stockpile 分块（矿石库存不是原地
> 资源量）、只采信报告自报的全矿口径；`analyze` 再按 ±5% 容差核一次，超差就以
> **报告自报合计**为准并在风险提示里说明——所以简报里出现的是它自报的数字，不是求和值。
