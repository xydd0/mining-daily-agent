...
`extract_text` 29.1 秒，合计约 56 秒，**超过 MCP 工具调用的默认超时（30 秒）**，于是
在 `client` 层的 `asyncio.timeout` 处被切断，走到调用期降级。

结果不算坏——风险提示会写明「调用 pdf.extract_resources 超时（超过 30.0 秒）」，
LLM 也会照实说「未提供具体资源量数字」，不会编造。但要真正拿到资源量，需要调大
`MCP_CALL_TIMEOUT_SECONDS`（配置项，默认 30）。

这条值得记住：**降级链路的正确性不保证数据的可获得性**。链路可以诚实地告诉你它失败了，
但那不等于它能拿到数据。
