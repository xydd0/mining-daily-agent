...
**取源顺序与「正文可用性」是有冲突的**：Google News 检索质量最好所以排第一，但它给的
中转页拿不到正文，而它一旦返回条目，后面两个直链源就不会被用到。**当前默认结果就是：
新闻条目齐、正文为空**。要让正文真正落地，把 Mining.com 提到第一位即可——代价是它的
检索面比 Google News 窄得多。这是一处明确的取舍，不是缺陷。

（曾试过把 Bing News 放首位换直链：其 RSS 端点在本机网络下不返回 RSS——`www.bing.com`
被 302 到 `cn.bing.com`，带不带 UA、加不加 `cc`/`mkt` 都落到首页 HTML。已回退。）
