...
- **挑 PDF 源按确定性排序**：真实 `.pdf` 链接 → `default_report_url()`（内置
...
- **表头单位只沿用到紧邻的下一块**。早先一路沿用到底，"Tonnes (Mt)" 会泄漏进后面几十块
  正文，把任意数字都变成吨位。
- **去重发生在解析器，交叉核对发生在 `analyze`，两处都要保持**：JORC 表把同一份
  资源量按 In-situ / Stockpiles / 全矿三种口径各列一遍（实测 Pilgangoora：
  436 + 9 + 445，逐块相加得 890，正好是真值 445 的两倍）。解析器剔除含 stockpile
  的分块、其余分块里只采信**自报合计最大**的那一块；`analyze` 再按
  `ResourceReport.reconciliation` 的 ±5% 容差核一遍，超差就报自报合计并留风险提示。
  解析器管选对块，`analyze` 管兜住它选错的情况。
- **`reconciliation` 由校验器从当前 `resources` 重算，不接受外部传入**：差额存下来
  就会随数据变化而过期，改一行就悄悄失真。
- **输出模板写死在 `nodes.py` 的 `_render_*` 里，LLM 只写新闻小节的小标题与导语**。
  小节名、编号对应、「矿石量 X Mt」这类措辞靠提示词保证不了，靠代码可以。改措辞就改
  `SECTION_*` 与对应的渲染函数，**不要**把格式交回给 LLM。
- **新闻先过滤再进简报**（`filter_relevant_news`）：只留讲主题主体的条目，剔除条数写进
  风险提示。判相关性用的是「有区分度」的主体词——`lithium`/`market` 这类泛词要排除，
  否则拿 "Pilbara" 检索回来的锂市场泛论会全部通过。
- **回溯天数有下限**（`MIN_PLAN_DAYS = 7`）：主题写「今日简报」时 LLM 会返回 `days=1`，
  实测一天的窗口搜回条数极少、过滤后新闻小节整个空掉。下限兜在代码里，不只写提示词。
- **多项目报告只取一张表**：内置年报有 Pilgangoora（445 Mt）与 Colina（70.9 Mt），
  逐块各自取最大仍会把两者相加（实测 515.9 Mt）。比较必须**全局**做，未计入的列进
  `ResourceReport.excluded_tables` 并写进风险提示。
- **表格块逐行解析，散文块才跨行切段**。按「类别词到下一个类别词」跨行切段时，
  Stockpiles 的 `Inferred ‑ ‑ ‑ ‑`（没有数值）会把下一行的 `Sub total 9` 抢来当吨位，
  凭空多出一条 9 Mt 的 Inferred。
- **自报合计行要结束当前分块**。真实年报把 Table 5（资源量）与 Table 6（储量）排在
  同一个块里、中间没有空行；储量表的行用 `Proved` / `Probable` 作类别词，解析器认不出、
  整行跳过，于是储量表的 `Sub total 207.2` 覆盖掉了资源量表的 `445`。**这个缺陷在
  fixture 上完全不可见**（fixture 恰好截到 "Table 6:" 为止）——只跑 fixture 会漏掉，
  改动解析器后要拿整份年报再跑一次。
